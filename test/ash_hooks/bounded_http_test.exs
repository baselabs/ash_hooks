    ...
    setup do
      ...
      {:ok, pid} =
        :inets.start(:httpd, [
          ...
          # server_root must be a REAL directory ("/tmp" does not exist
          # on Windows), and httpd's URL→path translation mangles
          # backslashes (every request 500s on Windows) — hand it
          # FORWARD slashes; file: accepts both forms on every OS and
          # unix paths are unchanged (tri-OS rule, 2026-09-16).
